...
class MovingCircle(BaseCircle):
    ...
    def get_center_position(self):
        ...
        return self.parent.get_swinger_position() + self.parent.radius * np.array([math.cos(self.parent.angle+parent_rot), math.sin(self.parent.angle+parent_rot)])
...
class CircleMachineVisualization(Visualization):
    def __init__(self):
        super().__init__()
        baseCircle = FixedCircle(pos=np.array([self.resolution[0] / 2, self.resolution[1] / 2]), radius=150, speed=np.pi*0.1, angle=0)
